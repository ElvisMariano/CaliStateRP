Citizen.CreateThread(function()
	...
	while CF == nil do 
		Citizen.Wait(0)
		CF = exports['cf'].GetFramework ~= nil and exports['cf'].GetFramework() or nil
	end

	local openMenu = function(namespace, name, data)
		for i=1, #Timeouts, 1 do
			CF.ClearTimeout(Timeouts[i])
		end
		...
		local timeoutId = CF.SetTimeout(200, function()
	...
	CF.UI.Menu.RegisterType(MenuType, openMenu, closeMenu)

	RegisterNUICallback('menu_submit', function(data, cb)
		local menu = CF.UI.Menu.GetOpened(MenuType, data._namespace, data._name)
		local post = true
		...
		if menu.submit ~= nil then
			...
			if tonumber(data.value) ~= nil then
				...
				-- Round float values
				data.value = CF.Math.Round(tonumber(data.value))
			...
			data.value = CF.Math.Trim(data.value)

			-- Don't post if the value is negative or if it's 0
			if post then
				menu.submit(data, menu)
			else
				CF.ShowNotification('That input is invalid!')
			end
		end
